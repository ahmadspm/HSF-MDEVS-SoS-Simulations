...
@Check
def checkIoTSFSModel(Constituent_System system) {
  ...
  for (StochasticConstraint stochasticConstraint : system.getAllStochasticConstraints()) {
    ...
    if (stochasticConstraint.askClause != null) {
      if (stochasticConstraint.askClause.variable == null) {
        error("The ask clause of a stochastic constraint does not have a variable", HybridStochasticFormalismPackage.Literals.STOCHASTIC_CONSTRAINT__TELL_CLAUSE)
      }

      if (stochasticConstraint.askClause.expression == null) {
        error("The ask clause of a stochastic constraint does not have an expression", HybridStochasticFormalismPackage.Literals.STOCHASTIC_CONSTRAINT__TELL_CLAUSE)
      }
    }
